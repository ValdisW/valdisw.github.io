...
function setup() {
   ...
   // mycircle = new circle(Starts[floor(random(0, 7))].x, y1, 500, 300, 1, 1, 2); //最后的参数是多少行多少列 指代到达的位置在图中的行列
    ...
    SPOTS=new circle(Starts[floor(random(0, 7))].x, y1, 0, 0, 1,floor(random(0, 3)), floor(random(0, 7)));
}
...
function show(mycircle) {
    ...
    if ((round(mycircle.pos.x) < round(J.x) && round(mycircle.pos.y) < round(I.y) && round(mycircle.pos.y) > round(H.y)) )
        mycircle.alive = false;       
    else
        mycircle.alive = true;

    if(round(mycircle.pos.x) == round(endx_1 + mycircle.col * interval_1)&&round(mycircle.pos.y)==round(endy_1 + mycircle.row * interval_2)){
        SPOTS.pos.x=Starts[floor(random(0, 7))].x;
        SPOTS.pos.y=y1;
        SPOTS.row=floor(random(0, 3));
        SPOTS.col=floor(random(0, 7));
    	//SPOTS=new circle(Starts[floor(random(0, 7))].x, y1, 0, 0, 1,floor(random(0, 3)), floor(random(0, 7)));
...
function circle(x, y, endx, endy, color, row, col) { //1 yellow ;2 blue
    ...
    this.update = function() {
		...
		var d = desired.mag();
		//if (d < 100) {
			var m = map(d, 0, 100, maxVel/3-1, maxVel/3);
			desired.setMag(m);
		//} else {
			//desired.setMag(maxVel/3);
		//}
